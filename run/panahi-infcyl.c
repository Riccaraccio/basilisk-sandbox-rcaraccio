...
int main() {
  ...
  double aspect_ratio = 30.; // aspect ratio of the particle
...
event output (t += 1e-3) {
  ...
  double radius = 0.;
  coord p;
  coord regionr[2] = {{L0/2,0},{L0/2, L0}};
  coord samplingr = {1, 1<<maxlevel};
  foreach_region (p, regionr, samplingr, reduction(+:radius))
    radius += f[];
