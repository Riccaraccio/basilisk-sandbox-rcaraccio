...
static void diffusion_boundary (Point point, int bid) {
  ...
  foreach_elem (YLList, jj) {
    ...
    double Dmix1v = Dmix1[];
#else
    double Dmix1v = mb.inDmix1[jj];
#endif
#if AXI
    mb.liq_massbdnow[jj] -= rho1v[]*Dmix1v*gradYL*Delta*dt*y;
#else
    mb.liq_massbdnow[jj] -= rho1v[]*Dmix1v*gradYL*Delta*dt;
#endif
  }
}
...
static void advection_boundary (Point point, int bid) {
  foreach_elem (YGList, jj) {
    ...
    mb.gas_massbdnow[jj] += rho2v[]*fluxYG*dt;
  }
  foreach_elem (YLList, jj) {
    scalar YL = YLList[jj];
    double fluxYL = face_flux_bid (point, YL, bid);
    mb.liq_massbdnow[jj] += rho1v[]*fluxYL*dt;
  }
  ...
  mb.totmass1bdnow += rho1v[]*face_flux_bid (point, U, bid, unity=true)*ff*dt;
  mb.totmass2bdnow += rho2v[]*face_flux_bid (point, U, bid, unity=true)*(1. - ff)*dt;
}
...
/**
### *compute_initial_state()*: compute the initial mass
*/

static void compute_initial_state (void) {

  if (!mb.totmass1init && !mb.totmass2init) {
    foreach(serial) {
      mb.totmass1init += rho1v[]*f[]*dv();
      mb.totmass2init += rho2v[]*(1. - f[])*dv();
    }
...
static void compute_balances (void) {
  ...
  foreach(serial) {
    ...
    foreach_elem (mb.mEvapList, jj) {
      ...
      mb.gas_mass[jj] += rho2v[]*YG[]*dv();
    }
    foreach_elem (mb.YLList, jj) {
      scalar YL = mb.YLList[jj];
      mb.liq_mass[jj] += rho1v[]*YL[]*dv();
    }
    mb.totmass1 += rho1v[]*f[]*dv();
    mb.totmass2 += rho2v[]*(1. - f[])*dv();
