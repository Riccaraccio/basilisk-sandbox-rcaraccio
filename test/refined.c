...
void prepare_grid (int maxlevel) {
  ...
  refine_unbalanced (level < maxlevel - 2, NULL);
  
  // refine along a circle
  refine_unbalanced (level < maxlevel && fabs(sqrt(sq(x - 0.5) + sq(y - 0.5)) - 0.2) < 0.05, NULL);
