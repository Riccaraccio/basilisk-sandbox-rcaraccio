...
static double xc = 0., yc = 0., Rc = 0.5;
...
event chemistry(i++) {  
  ...
  foreach() {
    if (f[] > F_ERR) {
      ...
      //porosity[] = 1 - (1 - porosity[])*exp(-omega[]*(1.-zeta[])*dt/rhoS); //direct integration, check equation
      porosity[] *= f[];
    }
  }
}
...
reset
set terminal svg size 450, 450
set output "mass-error-maxlevel.svg"
#set terminal epslatex size 3.6, 3.6 color colortext
