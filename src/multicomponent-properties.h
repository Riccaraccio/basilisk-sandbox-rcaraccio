...
void update_properties_initial (void) {
  foreach() {
    ...
    //rho1v0[] = rho1v[];
    cpSv[] = tp1.cpv (&ts1h);
    lambdaSv[] = tp1.lambdav (&ts1h);

    rho2v[] = tp2.rhov (&ts2h);
    //rho2v0[] = rho2v[];
    muGv[] = tp2.muv (&ts2h);
    cpGv[] = tp2.cpv (&ts2h);
    lambdaGv[] = tp2.lambdav (&ts2h);

    foreach_elem (Dmix2List, jj) {
      scalar DmixGv = Dmix2List[jj];
      DmixGv[] = tp2.diff (&ts2h, jj);
    }
  }

  boundary ({rhoSv,cpSv,lambdaSv,
      rhoGv, rhoGv0, muGv, cpGv, lambdaGv});
  boundary (Dmix2List);
}
